  ...
  document.getElementById('nebula-auth-submit').addEventListener('click', async () => {
    ...
    try {
      if (isSignUpMode) {
        ...
        // Show verification notice instead of closing
        const card = document.querySelector('.nebula-auth-card');
        card.innerHTML = `
          <div style="text-align:center; padding: 1rem 0;">
            <div style="font-size:3rem; margin-bottom:1rem;">📬</div>
            <h2 style="font-family:var(--font-primary,'Cormorant Garamond');font-size:1.75rem;margin-bottom:0.75rem;">Check Your Email</h2>
            <p style="color:#aaa;font-size:0.95rem;line-height:1.6;margin-bottom:1.5rem;">
              We've sent a verification link to <strong style="color:#fff;">${email}</strong>.<br>
              Please verify your email before signing in.
            </p>
            <button class="nebula-auth-btn nebula-auth-btn--primary" onclick="closeAuthModal()" style="margin-bottom:0.75rem;">Got it</button>
            <p style="color:#666;font-size:0.85rem;margin-top:1rem;">
              Didn't receive it? 
              <a href="#" onclick="resendVerificationEmail(event)" style="color:#fff;text-decoration:underline;">Resend email</a>
            </p>
          </div>
        `;
...
async function resendVerificationEmail(e) {
  ...
  if (USE_FIREBASE && auth && auth.currentUser) {
    try {
      ...
      alert('Could not resend email: ' + err.message);
    }
  }
}
...
function isEmailVerified() {
  ...
  return true; // localStorage fallback — no verification
}
...
async function approvePost(postId) {
  if (USE_FIREBASE && db) {
    await db.collection('posts').doc(postId).update({ status: 'published', approvedAt: new Date().toISOString() });
    return;
  }
  const posts = JSON.parse(localStorage.getItem('nebula_posts')) || [];
  const idx = posts.findIndex(p => p.id === postId);
  if (idx !== -1) { posts[idx].status = 'published'; localStorage.setItem('nebula_posts', JSON.stringify(posts)); }
}

async function rejectPost(postId, reason = '') {
  if (USE_FIREBASE && db) {
    await db.collection('posts').doc(postId).update({ status: 'rejected', rejectedAt: new Date().toISOString(), rejectReason: reason });
    return;
  }
  const posts = JSON.parse(localStorage.getItem('nebula_posts')) || [];
  const idx = posts.findIndex(p => p.id === postId);
  if (idx !== -1) { posts[idx].status = 'rejected'; posts[idx].rejectReason = reason; localStorage.setItem('nebula_posts', JSON.stringify(posts)); }
}
